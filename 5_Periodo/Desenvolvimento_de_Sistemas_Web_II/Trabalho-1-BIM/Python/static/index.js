...
function salvar(){
    ...
    let metodo;
    if (idCidadeAtual > 0){
        url = "http://127.0.0.1:5000/animal/"+idCidadeAtual;
        metodo = "PUT";
    } else { 
        url = "http://127.0.0.1:5000/animal";
        metodo = "POST";
    }
...
function listar(){
    ...
    fetch("http://127.0.0.1:5000/animal").then(resp => resp.json()).then(dados => mostrar(dados));
}
...
function alterar(idAnimal){
    idCidadeAtual = idAnimal;
    fetch("http://127.0.0.1:5000/animal/"+idAnimal).then(resp => resp.json()).then(dados =>{
        ...
        document.getElementById("biomas").value = dados.biomas;
        modalCadastro.show()
    });
}

function excluir(idAnimal){
    fetch("http://127.0.0.1:5000/animal/"+idAnimal, {method: "DELETE"}).then(function (){listar();});
}
